...
namespace Drapper.Configuration.Json
{
    ...
    public class JsonFileCommandReader : ICommandReader
    {
        ...
        private TypeSetting GetTypeSetting(
            ...
            Type type)
        {
            ...
            Require(namespaceSetting.Path != null,
                    ErrorMessages.NoTypeSettingAndNoPathInNamespace,
                    type.FullName,
                    namespaceSetting.Namespace);
            var path =
                $"{namespaceSetting.Path.Directory}\\{type.FullName}.{namespaceSetting.Path.Extension}";
            ...
            var result =
                JsonConvert.DeserializeObject<TypeSetting>(File.ReadAllText(path));
            Require(result != null, ErrorMessages.FileDeserializedToNull);

            return result;
        }

        private CommandSetting GetCommandSetting(
            ...
            string key)
        {
            ...
            var definitions =
                JsonConvert
                    .DeserializeObject<IDictionary<string, CommandSetting>>(
                        File.ReadAllText(path));
            Require(definitions != null,
